...
function validUserName (username) {
  if ( database.users.hasOwnProperty(username) ) {
    return true;
  } else {
    return false;
  }
...
function validArticleID (articleID) {
  if ( database.articles.hasOwnProperty(articleID) ) {
    return true;
  } else {
    return false;
  }
...
function validCommentID (CommentID) {
  if ( database.comments.hasOwnProperty(CommentID) ) {
    return true;
  } else {
    return false;
  }
...
function validBodyCommentStructure(parentObj) {
  if (  
        parentObj.hasOwnProperty('body') && 
        parentObj.body.hasOwnProperty('comment') &&
        parentObj.body.comment.hasOwnProperty('body') &&
        parentObj.body.comment.body &&
        parentObj.body.comment.hasOwnProperty('username') &&
        parentObj.body.comment.username &&
        parentObj.body.comment.hasOwnProperty('articleId') &&
        parentObj.body.comment.articleId
      ) {
        return true;
      } else {
        return false;
...
function validCommentStructure (articleID) {
  if ( database.articles.hasOwnProperty(articleID) ) {
    return true;
  } else {
    return false;
...
function createNewComment(url, request) {
  ...
  if  ( validBodyCommentStructure(request) && validUserName(request.body.comment.username) && validArticleID(request.body.comment.articleId) &&
        url && request) {
    ...
    //debug  && console.log("createNewComment");
    //debug  && console.log("request object ***");
...
function updateComment(url, request) {
  ...
  */
  ...
  if (!id || !request ) {
    response.status = 400;
  } else if (!validBodyCommentStructure(request)) {
    response.status = 400;
...
function deleteComment(url) {
  ...
  if (savedComment) {
    const existingUser = database.users[database.comments[id].username];
    const existingArtilce = database.articles[database.comments[id].articleId];
    // remove comment
    database.comments[id] = null;
    // remove comment id fron user
    existingUser.commentIds = existingUser.commentIds.filter(function(elem) {return elem != id;});
    // remove comment id from article
    existingArtilce.commentIds = existingUser.commentIds.filter(function(elem) {return elem !== id;});
...
function saveDatabase () {
  console.log("try to write database object to yaml file - databas.yml ");
  var yaml = require('js-yaml');
  var fs = require('fs');
  
  // save database object to a yaml document 
  var yamlDoc = yaml.safeDump(database);
  var fileContent = yamlDoc;
  var filepath = "database.yml";
...
function loadDatabase () {
  console.log("** try to read yaml file - database.yml and save to database object");
  var yaml = require('js-yaml');
  var fs   = require('fs');
